...
- 檔名範例：`nhi-P12074XXXX-20250517-20260517.json`
  - `P12074****` ＝ 身分證後 4 碼半遮罩（避免下載夾被瞄到，檔案內容仍是真實值）
...
| 身分證字號 | — | **不用填**，按下「取得」時 extension 會自動從健保存摺帶入；popup 顯示時會半遮罩成 `P12074****` |
| 姓名 | ❌ | 可填假；要在多病人 / 教學場景遮罩，到「⚙️ 進階設定 → 對外輸出時遮罩病人姓名」打開 |
| 完整生日 | ❌ | 可填假（只有出生年會影響檢驗值區間） |

按「**✓ 確定**」。資料只存在你的瀏覽器本機 (`chrome.storage.sync`)，**不會傳出去**。
