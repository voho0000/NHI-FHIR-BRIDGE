...
- **① 登入**：popup 偵測你是否已在健康存摺分頁登入；沒的話會帶你過去
- **② 您的資料**：填**性別 + 生日 + 姓名**（三項必填）→ 按「**儲存資料**」
  - 身分證字號**不用填**，按下取得時 extension 會自動從健康存摺 session 帶入
...
| 身分證字號 | — | **不用填**，按下「取得」時 extension 會自動從健康存摺帶入；popup 顯示時會半遮罩成 `P12345****` |
...
1. **健康存摺 session 過期**：回該分頁重新登入，再回 popup 按一次「取得健康存摺資料」
2. **日期範圍裡沒看病**：把 popup 步驟 ③ 的「日期範圍」拉長（例如最近 5 年、最近 10 年）再試

（身分證字號**不用手動填** — 自動從健康存摺 session 帶入。）
...
- ✅ 健康存摺 → 你電腦上的擴充功能 → 你電腦上的 backend → 你電腦上的 SQLite
...
2. 用內建的多層對照規則（`NHI_TO_LOINC` 醫令碼直對 + `PANEL_LOINC_MAP` panel 子項 + `LOINC_MAP` 顯示名對照，合計 200+ 條檢驗項目對 LOINC 的路由規則），把每個項目對應到 LOINC
...
- ⚠️ 健康存摺資料屬於敏感個資（PHI），請遵守《個人資料保護法》
