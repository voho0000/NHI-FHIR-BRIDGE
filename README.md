...
- **③ 取得**：按「**🔄 取得健保存摺資料**」
  - ~15–30 秒（看 NHI 速度，慢時可達 1-2 分鐘）後出現「**📥 下載健康紀錄檔**」按鈕
  - 按下會跳「另存新檔」對話框讓你選位置
...
如果是預設的 demo SMART app（醫析 MediPrisma · voho0000.github.io）：可能 GitHub Pages 暫時無法存取，等一下再試。
...
純 Extension 模式（沒 backend）：popup 上「📥 下載健康紀錄檔」旁邊的 🗑️ 圖示按鈕，按下後會清掉 `chrome.storage.session` 裡的暫存 bundle；下次 sync 會重新產生。Extension 本身沒長期儲存資料（暫存的 bundle 關掉瀏覽器自動清空 / 1 小時 TTL 自動清掉）。
...
- ✅ 暫存的 FHIR Bundle 存在 `chrome.storage.session`（**關閉瀏覽器自動清空**），且 **1 小時 TTL 自動清掃**，**user 點下載完成的瞬間立刻清除**
...
- ✅ FHIR PHI 端點走 SMART OAuth2 Bearer + PKCE；patient-scoped token 只能讀自己的資料
- ✅ Bundle 內 `Patient.id = SHA-1(身分證)`，URL 路徑不洩漏原始 ID（原始 ID 只在 `Patient.identifier`）
