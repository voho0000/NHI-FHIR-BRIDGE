...
|---|---|---|
| `src/nhi-adapters.ts` | Each NHI JSON shape → normalized FHIR-friendly shape | `tests/adapters.test.js` (60+ cases) + `tests/fixtures.snapshot.test.js` |
| `src/nhi-endpoints.ts` | Endpoint URL → page_type → adapter registry + 中文 label map | `tests/endpoints.test.js` |
...
inside `background.ts` next to `chrome.scripting.executeScript` calls,
...
1. Write the adapter in `src/nhi-adapters.ts`. Export it.
2. Register it in `src/nhi-endpoints.ts` with a Chinese label (the
