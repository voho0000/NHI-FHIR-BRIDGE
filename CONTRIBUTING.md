...
# 設定環境變數
cp ../.env.example ../.env
# 編輯 .env 把 SECRET_KEY 改成至少 32 字元的隨機字串
# 產生 key：python -c 'import secrets; print(secrets.token_urlsafe(48))'

# 建立資料庫
SECRET_KEY="你剛產生的 key" alembic upgrade head

# 啟動 dev server
SECRET_KEY="你剛產生的 key" uvicorn app.main:app --reload --port 8010
```
...
如果你發現安全弱點（例如可繞過 SECRET_KEY 驗證、資料外洩風險、XSS），請**不要**在公開 Issue 揭露，先以 email 私下聯繫 voho0000 @ gmail.com。
