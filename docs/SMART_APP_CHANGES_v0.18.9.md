...
經 2026-06-13 對健康存摺實際 API 逐欄位查核：NHI 的處方藥品明細端點（IHKE3306S02）**不提供結構化用法／頻次／劑量**（回傳欄位只有藥品代碼、藥名、給藥日數、數量、藥理分類）。因此 `MedicationRequest.dosageInstruction` 維持空白屬正常，**並非 bridge 漏抓**。若貴 App 有顯示用法的需求，此資料源頭即無。
