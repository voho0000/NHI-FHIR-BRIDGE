...
**Q5: 安裝時 Chrome 跳出「讀取以及變更你在 myhealthbank.nhi.gov.tw 的資料」，這樣安全嗎？**
這是 Chrome **對所有「能存取某網站」的擴充功能都會顯示的固定字眼** — 不是 Bridge 特別要求的，開發者也改不了這行字。它描述的是「這個權限**技術上能做什麼**」（網站存取權同時含讀與寫），不是「Bridge 實際做了什麼」。

實際上 Bridge **只「讀取」**你健康存摺的資料，在你電腦上轉成 FHIR 檔，**從不修改健保署網站上的任何內容**。這點你可以自己驗證：本工具開源，所有程式碼公開可查（見下方「透明度」）。Chrome 之所以保守地寫「以及變更」，是因為它無法逐一判斷每個擴充功能的實際行為，只能按權限的最大能力警告。
