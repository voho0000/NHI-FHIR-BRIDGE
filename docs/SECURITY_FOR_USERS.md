...
**資料只會在你自己的電腦內處理；Bridge 不會把資料傳到開發者的伺服器，也不會送進任何 AI / 雲端服務。**

（唯一例外：你若在下載後**主動**開啟外部工具「醫析 MediPrisma」並使用它的「AI 問答」，資料才會被送到該工具的雲端 LLM —— 那是該第三方工具的選用功能，與 Bridge 無關，詳見後段。）
...
**Q2: 我能不能完全離線使用？**
擴充功能本身可以離線，但你**必須連線才能抓健康存摺**（健保署 API 在雲端）。一旦資料到你瀏覽器，就可以離線了，後續轉換 + 下載都不需要網路。

**Q3: 我擔心健保署看到我用 bridge → 影響我什麼權益**
你登入健康存摺、按那些按鈕，跟你直接用網頁上的功能，健保署看到的 API call 是一模一樣的。Bridge **不會**對健保伺服器留特殊指紋。
...
**重點回顧**：健康資料停留在你的瀏覽器內，除非你主動「下載到電腦」、或者主動開啟 SMART App 並使用其 AI 功能。Bridge 在整個流程中的角色，是把健康存摺資料從健保署網站轉換並搬運到你電腦的工具。
