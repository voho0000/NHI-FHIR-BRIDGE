...
病人(探針病例)實際只有 35 筆檢驗,B/C 段這些低頻專科碼**都不在其健康存摺**,故無法用真實 row 反查 NHI 項目名稱。改以 loinc.org 親自複驗 + 臨床判斷處理,逐碼自驗替代碼(rule #5):
...
**12195B Her-2/neu — 改為留空(unmapped)**:現碼 `18474-7` 是 HER2 **IHC**(蛋白染色),但 bridge 註解寫 **ISH**(基因擴增)—— 兩者是不同 LOINC,而健康存摺無此 row 可確認 NHI 實際 billing 哪一種。依使用者原則「**不確定正確性寧可不發 LOINC**」(2026-06-14),移除 `18474-7`,落到 NHI-code-only + 原始顯示。待 NHI 項目名稱(ISH vs IHC)確認後再補。
