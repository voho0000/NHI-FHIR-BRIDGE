...
| **Category** | **Productivity**（本工具主要面向一般民眾做個人健康紀錄備份，比 Developer Tools 更貼近 end user） |
| **Language** | 繁體中文（zh-TW）|
...
```
NHI-FHIR Bridge 把你在健保署「健康存摺」(myhealthbank.nhi.gov.tw) 上的個人醫療紀錄，一鍵轉換為 HL7 FHIR R4 國際標準格式，方便你做個人健康資料備份、研究分析、或匯入其他能讀 FHIR 的軟體（例如 SMART on FHIR App）。

▍它能做什麼
• 擷取以下健康存摺資料並轉為 FHIR：
  - 就醫紀錄（門診、急診、住院）→ Encounter
  - 用藥紀錄（含慢性處方箋）→ MedicationRequest
  - 檢驗檢查結果 → DiagnosticReport + Observation（內建多層 LOINC 路由規則：NHI 醫令碼、panel 子項、顯示名稱對照）
...
• 兩種使用模式：
  - 純 Extension：擷取資料 → 下載成 FHIR Bundle JSON 檔
  - Extension + Backend：搭配自架本機 FHIR 後端 + Dashboard + SMART App 整合

▍隱私與安全
• Bridge 本身零資料收集：開發者無法存取你的健康資料
• Bridge 本身不使用 AI、LLM、雲端或 telemetry
• 資料預設只在你的瀏覽器與電腦處理；若你主動下載、上傳自架後端，或另開外部工具／AI，則依該目的地處理
• 開放原始碼 (Apache License 2.0)，可自行檢查所有程式碼

▍關於安裝時的權限提示
安裝時 Chrome 會顯示「讀取以及變更你在 myhealthbank.nhi.gov.tw 的資料」—— 這是 Chrome 對「所有能存取網站的擴充功能」都會顯示的固定字眼，描述的是權限「技術上能做什麼」（網站存取權同時含讀與寫），不是本工具實際的行為。實際上本工具只「讀取」你健康存摺的資料並在本機轉檔，從不修改健保署網站上的任何內容；開源可查證。

▍使用前提
• 你必須是健保署「健康存摺」的合法使用者（用自己的健保卡登入）
• 一次只擷取「健康存摺目前切換顯示的那個人」的紀錄；只能擷取你有權檢視的資料——本人，或經健保署眷屬功能同意綁定的眷屬
...
• Lab and exam results → DiagnosticReport + Observation (multi-layer LOINC routing: NHI order code, panel sub-analyte, display-name maps)
...
• Bridge itself collects zero data: the developer cannot access your health data
• Bridge itself uses no AI, LLM, cloud, or telemetry
• Data is processed only in your browser/computer by default; if you choose to download it, upload to your self-hosted backend, or open an external tool/AI, it's handled per that destination
• Open source under Apache License 2.0

▍Prerequisites
• You must be a legitimate user of Taiwan's NHI "My Health Bank" (logged in with your own NHI card)
• Captures one person per sync — whoever "My Health Bank" is currently switched to. You may only capture records you are authorized to view: your own, or a dependent you are legitimately bound to via NHI's dependents (眷屬) feature
...
| `storage` | 在本機儲存使用者偏好設定（性別、出生年）、同步進度，以及暫存待下載的健康紀錄檔（下載完成、使用者清除、或 1 小時 TTL 到期即刪除）；啟用「一併下載影像」時並暫存 NHI session bearer token 供背景抓圖（30 分鐘 TTL 到期即清除）。資料僅存在 chrome.storage，不傳送至任何外部伺服器。 |
...
| `alarms` | 維持 Chrome Manifest V3 service worker 在長時間同步（30 秒以上）過程中不被回收，確保資料完整擷取；並每 10 分鐘執行清掃，刪除逾時（1 小時）的本機暫存健康紀錄檔。 |
...
| Authentication information | ✅ 勾 | 不讀密碼、不碰登入 form；但啟用「一併下載影像」時會本機暫存 NHI session bearer token（背景抓圖用），30 分鐘 TTL 到期／流程結束清除，不外傳 |
| Personal communications | ❌ 不勾 | 無 |
| Location | ❌ 不勾 | 無 |
| Web history | ❌ 不勾 | 無 |
| User activity | ❌ 不勾 | 無 |
| Website content | ✅ 勾 | 只讀取 myhealthbank.nhi.gov.tw 健康存摺頁面／API 內容，不讀一般瀏覽內容 |
...
| 選項 | 建議 |
|------|------|
| **Visibility** | Unlisted（推薦）— 不會出現在搜尋結果，只有拿到網址的人能裝。降低陌生使用者安裝與支援壓力 |
