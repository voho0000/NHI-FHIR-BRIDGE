# SMART on FHIR 對接變更通知 — v0.18.8（2026-06-13）
...
> ### ⚠️ 最低 bundle 版本：**0.18.8**
> 本通知描述的兩項變更**需 bundle 版本 ≥ 0.18.8**，請以此為準測試。
> - 癌症篩檢的 **Observation** 本身在 0.18.7 即會出現，但用來分類的 `source-program` `meta.tag` 因一個分組路徑的瑕疵，**在 0.18.7 的實際健康紀錄檔中沒有產生**，到 **0.18.8 才修正**。本通知請對接方「以該 tag 為唯一篩選依據」，因此**務必用 0.18.8（含）以後的 bundle 測試**。
> - bundle 版本可從 `Bundle.meta.tag` 讀取：`{system:".../bridge-version", code:"0.18.8"}`。
> - 已於真實健康紀錄檔驗證（2026-06-13，v0.18.8）：癌篩 4 筆、成人健檢 29 筆皆帶正確 `source-program` tag。
