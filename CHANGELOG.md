...
- 對接 App 影響：同次就診的用藥/檢驗 `encounter` 參照更準;入院當天會多出一筆門診/急診 Encounter(與健保存摺 UI 一致)。
...
- **按鈕改成柔和的 tonal 藍**：原本實心飽和藍的主要按鈕（儲存、取得健保存摺資料、下載…）改成淺藍底＋藍字＋藍邊，跟「開啟 App」按鈕、選中狀態、提示框同一個柔和色系；不再有一塊跳色的實心藍。
...
- **NHI 登入 token 用完即清**：健保存摺的 session token 快照過期後現在會主動從磁碟刪除（過期讀取自清 + 定時清掃 + 輪詢結束硬清），不再殘留。
...
住院紀錄現在會一併抓取健保存摺「查看檔案」裡的出院病摘，每次有病摘的住院輸出一份 FHIR R4 DocumentReference：
...
popup 新增 opt-in「抓影像檢查報告」開關：勾選後，健保存摺裡的影像圖片會以 base64 JPG 附件放進 FHIR 報告（DiagnosticReport.presentedForm），與文字報告合體。
...
- 同版一併納入 **照護計畫（CarePlan）**：健保存摺「我參與的照護計畫」對應為 FHIR R4 CarePlan，收案／結案日期、醫院、計畫名稱忠實搬運
