...
## 1.0.7 — 2026-06-27（影像功能 UX 整理:不再誤報「備製中」+ 明細數字對得起來 + 取得影像更清楚）

純擷取／顯示端調整,不改任何病人數值。

▍不再出現假的「部分影像仍在備製中」提醒
- 先前即使影像都抓到了、明細各項都是 0,總結列仍可能跳「部分影像仍在健康存摺備製中…」。現在這條提醒只在「下次取得真的能補齊」時出現(等候備齊／前次等候／抓取失敗);明細全 0 就絕不再提醒。
- 修正一個會讓「前次等候」一直掛著不消的殘留:有些檢查健保只提供 DICOM 原始檔、不出 JPG(例如臺北榮總),這種列以前每次同步都重新排隊、無限「前次等候」。現在會正確清掉。

▍影像明細數字看得懂、對得起來
- 改成「影像檢查：N 件（健保 M 筆上傳列）」。健保把一次檢查拆成「報告／影像」多個上傳列,所以清單列數會比實際檢查數多;現在兩個數字都列出來。
- 明確標示「僅 DICOM 原始檔 X 筆（無 JPG、未下載）」—— 有些檢查健保只存醫療級 DICOM、無 JPG 預覽,bridge 只處理 JPG 不下載 DICOM;但**報告文字仍完整擷取**(沒有遺漏該次檢查)。

▍取得影像的操作更清楚
- 「一併下載」標示 **BETA**(影像功能仍在開發中、健保 JPG 備製不穩定)。
- 勾選後出現精簡提示 + 「前往影像頁」連結:第一次抓某位病人的影像,建議先到影像頁讓健保開始備製、約 1 分鐘後再取得;產生檔案後此提示自動收起。
- 取得後若有影像沒拿到,結果區提供「開啟影像清單頁」按鈕,直接帶你查看。

▍其他
- 修正明細偶爾「跑版」(備註行被冒號切成兩欄、中文一字一行)。

## 1.0.6 — 2026-06-25（去識別化更完整 + 出院病摘抓取更可靠 + 修正門診手術被誤判成住院）

接續 1.0.5 的資料正確性與隱私強化。純擷取／轉檔端修正,不改任何病人數值。

▍去識別化(開啟時)更完整 —— 不再因「設定裡的姓名打錯」而漏遮
- **姓名**:病摘／病理報告內文的姓名改用**結構位置**遮罩(夾在「病患資訊：…性別 年齡」之間、或「病患姓名：」欄)。**就算你在設定裡把名字打錯,也照樣遮成「王O明」**(先前只比對你輸入的字串,打錯就漏)。
- **地址**:出院病摘的住家地址 → `[已去識別]`。
- **病歷號**:報告裡沒有「病歷號」三個字、只夾在「性別 年齡」前的那組號碼,也會被遮(工作號／醫師名等非病人識別資訊保留)。

▍出院病摘抓取更可靠
- 改成**逐筆、放慢**抓取,避開健保署對快速連發的限流 —— 先前重症(住院很多)病人會**靜默漏抓**幾份病摘而不自知;實測一位 23 筆住院病摘從「漏 6 筆」變成「23/23 全抓到」。
- 加入失敗重試;若仍有失敗,明細會**顯示原因(登入過期／限流／逾時)並提示可重抓**。

▍修正:門診手術被誤判成「住院」
- 先前會把某些**門診手術／處置**誤列成一筆「住院」(實際健保署住院清單並沒有這筆)。現在住院**只採認正式住院清單**;門診手術維持為處置紀錄,不再無中生有住院。

## 1.0.5 — 2026-06-23（就醫合併更精準 + 用藥關聯到正確的那次就診）

修正一個會**遺失診斷**的就醫合併錯誤,並讓用藥/處置/檢驗都關聯到正確的就診。純資料結構修正,不改任何病人數值/日期/醫院(忠實搬運)。

- **同一天多次就診不再被錯誤合併**:先前同日、同院的不同就診(不同科別/不同診斷/不同部分負擔)可能被併成一筆,導致其中一個診斷整個消失(實例:同日「慢性腎臟病 stage 3b」門診與「咳嗽」急診被併掉 → CKD 不見了)。現在只有 **日期＋醫院＋主診斷＋全部次診斷＋部分負擔＋申請點數 全部相同** 才視為同一次就診。
- **次診斷完整保留**:住院/門診的次診斷(住院可達 10＋筆)都寫入該次就診資源。
- **用藥關聯到「開這個藥的那次就診」**:依該次就診實際開立/使用的藥品清單關聯 —— 門診用門診處方清單、住院用住院給藥清單。關聯不到任何一次就診的藥(到藥局領藥/自備藥 —— 健保申報不會把它列在就診裡)**維持獨立、不亂塞**;同一個藥同日同時出現在急診與住院,也會正確各自歸位。
- **處置/檢驗報告/血壓 同類合併錯誤一併修正**:不同醫院、不同主診斷、不同數值的項目不再被誤併(血壓同日多次測量也都各自保留)。

## 1.0.4 — 2026-06-22（影像下載:不再誤報「無影像檔」+ 一鍵連結 + session 偵測）

當勾選下載影像、卻一張圖都沒抓到時,先前版本一律寫「無影像檔」—— 但這往往不是真的沒影像:健康存摺的 JPG 預覽是從永久的原始檔（DCM）即時生成、只快取一段時間,過期或冷卻時整批顯示不出來（原始 DCM 仍在）。本版把這段訊息改誠實,並讓使用者一鍵自助:

- **訊息改誠實**:抓到 0 張圖但有影像列時,不再宣稱「無影像檔」,改提示使用者開「影像清單」頁查看該筆是否顯示「**有影像檔**」—— 有的話稍候約 1 分鐘再重新取得即可;顯示「無影像檔」則目前沒有影像圖片可下載。
- **一鍵「🖼️ 開啟影像清單頁」chip**:把你**現有已登入的健康存摺分頁**導到影像清單頁（不開空白新分頁 —— 新分頁沒有 token 會被踢到登入頁）,讓你自己重新檢視 / 重抓。
- **session 逾時偵測**:點 chip 時若該分頁已登出 / 閒置逾時,改顯示「請先重新登入再點一次」,不再默默把你丟到登入頁;找不到健康存摺分頁也會提示先登入。
- 純擴充功能端變更;不影響其他資料（就醫/用藥/檢驗等）與其他功能。
...
- **隱私優先**:Bridge 本身零資料收集、不接 AI/雲端;去識別化**預設開啟**(姓名/身分證/生日遮罩,病摘與報告內文一併去識別)。
...
**根因**(2026-06-17 實機驗證):健保署是「**有人真的開啟/渲染 IHKE3408S01 影像清單頁,後端才開始確認那批影像**」。橋接原本是在使用者目前的分頁(通常停在基本資料頁)用 API 輪詢影像清單 30 秒 —— 但**純打 API 不會觸發 NHI 去確認**,所以那些「-」永遠不會變,30 秒空轉後就放棄。(這也說明為何之前「過幾分鐘自己會好」的觀察成立 —— 前提是使用者**曾經手動點進影像清單一次**。)

實測:同一個全新病人,在基本資料頁時 27 列「-」、0 可抓;把頁面導到影像清單後 → 0 列「-」、**11 列可抓** + 20 列無影像。

**修法**:當影像清單整批還在「資料準備中」時,橋接先用一個**隱藏分頁**渲染一次 IHKE3408S01(注入 token、等 SPA 載入)來觸發 NHI 確認 —— 不打擾使用者目前的分頁;確認是**病人層級**的,所以渲染一次後即可輪詢到狀態變化。輪詢預算由 30 秒拉長到約 60 秒(已被觸發、會收斂)。若影像很多、一次同步內還沒全部確認完,維持「過幾分鐘再同步一次」即可補齊(觸發狀態 NHI 端會保留)。
...
- **移除整套 live banner ＋ 背景輪詢**:刪 `imaging-prep-banner.ts`、`imaging-prep-poll.ts`,以及對應的 popup markup/CSS、`IMAGING_PREP_*` 常數、`alarms` 輪詢、`startPrepPolling`/`stopPrepPolling`、banner 專用圖示(共 9 檔)。
- **改成不帶數字的靜態提醒**:同步結束若有影像仍在備製/確認/未抓到,摘要只附一句「**部分影像仍在健康存摺備製中,請過幾分鐘後再按「取得健康存摺資料」即可補齊**」(不再顯示不可靠的即時張數)。`查看明細` 仍保留「資料確認中 N 筆 / 無影像檔 N 筆」分類(這 count 是同步當下直接數的,準)。
- **同步內「確認影像清單」等待 60s → 30s**:仍一偵測到 usable 列就早退;30s 只在清單真的還在確認時才用滿,避免把其他資料明細卡住。慢的(大量影像史)清單靠**使用者手動再同步**取得(那句提醒會講)。
...
- **用詞統一為「健康存摺」**:popup 與 SW 狀態訊息先前混用多種寫法(「健保署」與存摺類俗稱),官方正式名為「健康存摺」。本次把所有**使用者可見**文字統一成「健康存摺」(popup 全部 ＋ 影像 banner ＋ SW 進度/逾時訊息);文件、README 與內部開發註解的用詞亦隨後一併統一(「健保署」指機關時維持不變)。
...
- **「按鈕為何不能按」的原因改成 info chip**:原本是 `→ 健康存摺分頁尚未登入　回 ① 登入 →` 的雙箭頭＋整排藍底線 strip,讀起來像 debug 文字、跟卡片式 UI 不搭。改為柔藍填色的圓角 chip(前置圖示＋原因文字＋右側 chevron,整條可點跳到要修的步驟):登入狀態用**鎖頭**圖示、其他原因(切到分頁/基本資料未完成/生日格式)用 **info** 圖示。圖示為 inline SVG(非 emoji)。
- **登入提醒不再是紅色警告框**:點同步但未登入時,先前用 `.status.error`(紅框＋🔒)顯示「還沒登入」。同一件事卻有兩種長相(藍色 chip vs 紅框)。改為柔和的 `.status.info`(藍色左條),文案與 chip 對齊成「健康存摺分頁尚未登入 — 回 ① 登入」,**移除 🔒**(🔒 之後只保留給底部安全保證)。
...
- 影響:像長庚嘉義 2/11(J18.9 肺炎)、1/28(U07.1 COVID)這類「入院當天門診/急診與住院同診斷」的住院,用藥現在會正確掛回該次住院(先前 bridge 有抓到藥、卻因診斷分不開而全部不掛)。診斷比對降級為最後備援(主要給沒有型別的處置等用)。型別僅作內部關聯訊號,不寫進 FHIR 輸出。

## 0.20.2 重點 — 2026-06-16（住院療程用藥掛回該次住院）

- **住院用藥靠療程窗(validityPeriod)精準掛回住院**:v0.20.0 的就醫消歧靠「唯一診斷命中」,但常見情況是**入院當天的門診/急診與住院共用同一個主診斷**(病人因肺炎就醫 → 因肺炎收住院),導致藥的診斷同時命中門診與住院 → 消歧失敗 → 用藥變成「不掛任何就醫」。對接 App 若以就醫顯示用藥,該次住院就看不到藥(實際 bridge 有抓到,只是沒串上)。修正:消歧時**最優先**比對藥的 `dispenseRequest.validityPeriod`(=入院～出院),與住院 Encounter 的期間一致就直接掛該次住院。真實案例:長庚嘉義 2/11(J18.9 肺炎,22 筆)、1/28(U07.1 COVID,14 筆)的住院療程藥現在正確掛回住院。
- 註:同日門診/急診的**單日**處方(無 validityPeriod)且與住院共用診斷者,仍保守留空(不亂掛),待更強訊號再處理。
...
源自長庚嘉義 5/18 案例稽核：入院當天的急診就醫(K92.0 吐血)整個從 bundle 消失,連帶用藥/檢驗被錯掛。

**就醫(Encounter)**
- **不再刪除入院當天的門診/急診**：舊版 `dedupAdmissionDayAmb` 假設「就醫清單(IHKE3303)含住院、與住院紀錄(IHKE3309)重複」,故刪入院日同院的門診。**前提是錯的** —— 就醫清單只有門診/急診/藥局(不含住院,2026-06-15 線上查證),兩者不重疊。那筆被刪的其實是「病人就醫 → 被醫師收治住院」的**源頭就醫**。已移除此去重;真住院重複本就由 Encounter id(含 class)自動合併。
- **急診分類修正**：就醫型別常被報成「西醫」即使實為急診。現在會看處置清單(`sp_IHKE3302S05`)有無「急診(按檢傷分類)診察費/護理費」,有就正確分類為 **EMER**(急診)。

**用藥 / 檢驗 → 就醫的精準掛載**
- 同一天同院若同時有門診/急診 + 住院兩筆就醫,**用藥靠自身診斷**掛回正確就醫(急診藥 dx K92.0 → 急診、住院藥 dx R04.2 → 住院);**檢驗靠採檢日**分流(入院當天 workup → 源頭門診/急診,住院後續日 → 住院)。只在唯一命中時掛,模糊則留空不亂猜。
- 對接 App 影響：同次就診的用藥/檢驗 `encounter` 參照更準;入院當天會多出一筆門診/急診 Encounter(與健康存摺 UI 一致)。
...
- **服務背景被瀏覽器中止時的復原**：Manifest V3 service worker 若在取得途中被系統回收,先前 UI 會永遠卡在「取得中…」;現在偵測到逾時會自動復原並提示重新取得。
...
- **對接 App 注意**：`Procedure.note` 不再帶診斷理由，請改讀 `reasonCode`。
...
- **對接 App 注意**：這是 `Procedure` 的破壞性變更 —— `Procedure.id` 會改變（改由醫令碼派生）、`code.text` 由分類名變手術名、資源數量可能變多。其它資源型別不受影響。

## 0.18.13 重點 — 2026-06-14

**一輪介面一致化／和諧化（純 UI，不影響任何資料、對應或流程）。**

- **二元選項統一成「分段切換」**：影像下載（不下載／一併下載）、去識別化（關閉／開啟）都改用跟「輸出方式」同一個分段切換元件，取代原本散落的裸 checkbox。
- **Step 2 表單改成跟 Step 3 一致的版型**：生日／性別／姓名／去識別化改成「左邊標籤、右邊填寫」的橫向排列（生日與性別不再擠同一行），兩個步驟現在是同一套視覺。
- **按鈕改成柔和的 tonal 藍**：原本實心飽和藍的主要按鈕（儲存、取得健康存摺資料、下載…）改成淺藍底＋藍字＋藍邊，跟「開啟 App」按鈕、選中狀態、提示框同一個柔和色系；不再有一塊跳色的實心藍。
- **說明只在需要時展開**：去識別化的遮罩細節＋警告、影像的「JPG≠DICOM」提醒，都改成選了對應選項才出現，平常收起讓卡片清爽。
- Step 2 動作按鈕（儲存／清除）加上一條 footer 分隔線，與表單內容分區。
...
- 流程教訓已寫入專案規則：找「錯碼」可靠，但**建議的「正確替代碼」必須親自 WebFetch 複驗**(本輪即抓到先前審查 agent 建議的 2 個替代碼其實是錯的)。
...
- **通用防護**:檢驗 panel 底下「對不到碼」的子項目,以後不再繼承整份 panel 的 LOINC(避免把 panel 碼誤掛到單一分析物);改為只保留 NHI 醫令碼＋原始名稱。panel 報告本身仍正確帶 panel LOINC。
...
- 用藥「用法／頻次」：實測確認健康存摺此端點不提供結構化用法，已在程式註明（非缺陷，是 NHI 資料本身沒有）。
...
- **修「同步中無法切到第 ② 步看資料」**：取得健康存摺資料的過程中，若切到「② 您的資料」查看，約一兩秒就會被自動拉回「③ 取得資料」的進度畫面。原因是同步每隔約 1-2 秒寫一次進度，每次都把畫面強制跳回第 ③ 步。現在同步進行中的進度更新不再搶走你的畫面；只有「開始同步」「同步完成／失敗」「重開視窗接回進行中的同步」才會跳到第 ③ 步。
...
- **NHI 登入 token 用完即清**：健康存摺的 session token 快照過期後現在會主動從磁碟刪除（過期讀取自清 + 定時清掃 + 輪詢結束硬清），不再殘留。
- **本機伺服器（backend）安全收緊**：未設 API key 時，CORS 只允許本機來源（不再預設放行 GitHub Pages 與任意 extension）；新增 Host header 驗證，防 DNS rebinding 繞過；API key 與 PKCE 比對改用 timing-safe 比較。
- **去識別化 Patient.id 修正**：勾選去識別化時，Patient.id 的雜湊改用半遮身分證（F12345XXXX）計算 — 舊版用完整身分證雜湊，可被暴力反推。
- **隱私文件更正**：PRIVACY / SECURITY_FOR_USERS / README 改為如實描述暫存 bundle 的實際存放位置與清除時機（非「關瀏覽器即清」），補上 `unlimitedStorage` 權限說明與 keyless 模式的限制聲明。
- **連線測試與 API key 錯誤提示改善**：API key 設錯不再顯示假的「🟢 已連線」，改顯示明確的「金鑰錯誤」指引；backend URL 加上格式驗證（限 http/https），非本機的 http 連線會持續顯示明文傳輸警告。

---

## 0.18.3 重點 — 2026-06-12

**🖼️ 影像準備狀態機修正 + 後端上傳去識別化**

使用者實測回報的連續修正，主要解決「影像永遠等不到」的幾種卡死情況：

- **幽靈影像無限迴圈修掉**：有些影像健保署其實備不出來（觸發後狀態退回原點），舊版誤判為「✅ 影像已備齊」誘導使用者一直重抓。現在以「可下載張數有沒有真的變多」判定，備不出來就誠實顯示「部分影像健保署無法提供」並停止輪詢。
- **影像清單確認更耐錯**：單次查詢失敗不再讓整個流程崩潰，改為記錄後重試。
- **登入逾時 banner 不再卡死**：逾時/登入過期兩種狀態補上一鍵重跑按鈕。
- **30 分鐘等候上限改由 popup 自行計算**：電腦休眠或瀏覽器回收背景程式也不會再出現「已等候 139 分鐘」的凍結畫面。
- **後端上傳模式也支援去識別化**（延續 v0.18.1）：勾選後上傳到自己伺服器的資料同樣遮身分證／生日／姓名，預設關閉。

---

## 0.18.2 重點 — 2026-06-12

**🧪 修 06012C「尿液一般檢查」整組驗尿被標成單一「尿液顏色」的靜默錯誤**

06012C 是 18 項的尿液常規檢查 panel，舊版把全部 18 個子項目都塌成 LOINC「Color of Urine（尿液顏色）」— FHIR 驗證器驗得過、程式不報錯，只有人工核對才看得到（silent bug）。本版：

- 06012C 升級為 panel 處理，與 06013C 共用同一張子項目對應表（單一真相來源，不會再各自漂移）
- 新增 6 個尿沉渣顯微鏡項目的 LOINC（上皮細胞／白血球計數／紅血球計數／細菌／結晶），每個都經 loinc.org 查證
- 實測：18 個子項目 → 18 種正確 LOINC（原本只有 1 種）；血清的尿酸／尿素氮／血糖不受影響
- 新增 CI 防回歸測試 23 條；並把「NHI 碼要先確認是單項還是 panel」寫進專案規則（rule #10）

---

## 0.18.1 重點 — 2026-06-11

**🕶️ 去識別化補齊 — 身分證半遮 + 生日只留年份**

舊版勾「遮罩姓名」時檔名與姓名有遮，但 bundle 內容的身分證與生日還是完整的 — 分享、展示、測試時是實質隱私風險。本版把這個開關升級成完整去識別化（預設關閉，不勾選時忠實搬運不變）：

- Patient 的身分證 → 半遮（F22154XXXX，與檔名一致）；生日 → 只留年份（月日歸一為 01-01，年齡誤差 ≤1 歲）
- 報告抬頭文字裡的身分證也一併遮掉
- popup 文案改為「輸出時去識別化（姓名／身分證／生日）」，並加上誠實警語：仍含醫院與就醫日期，屬有限去識別，勿當完全匿名公開散布

---

## 0.18.0 重點 — 2026-06-08

**🏷️ 沒有 LOINC 的檢驗，名稱改取英文檢驗項目名 + 參考值 HTML 符號修正**

- 沒有 LOINC 對應的檢驗（如 12068C aTG）的 `code.text` 改優先採用醫院 LIS 的英文檢驗名（assaY_ITEM_NAME），與有 LOINC 的檢驗顯示風格一致；中文醫令名不丟，仍完整保留在 NHI coding 裡
- 參考值文字裡的 HTML 符號（`&lt;` → `<`）解碼還原，結構化上下限同步補齊

---

## 0.17.8 重點 — 2026-06-08

**🩹 FHIR 對應四項修正 — ICD 補點／LOINC 名稱／參考值疊字／用藥用法文字**

第三方 FHIR 稽核後的修正：

- **ICD-10 補點統一**：就醫紀錄（Encounter）的診斷碼之前沒補小數點（E079），與用藥紀錄的 E07.9 不一致，現在統一補點
- **LOINC 20454-5 顯示名修正**：尿蛋白試紙（定性）誤用了定量檢驗的名稱，改為官方正式名 "Protein [Presence] in Urine by Test strip"
- **參考值左右相同的疊字收斂**：`[(-)][(-)]` → `(-)`
- **用藥用法改用看得懂的中文**：「給藥總量 N，給藥日數 M 天（平均每日 R）」，不再捏造 NHI 沒給的 "dose" 單位

---

## 0.17.7 重點 — 2026-06-08

**🖼️ 影像清單「還沒準備好」的判定改進**

首次抓影像時，NHI 清單有時先只回一列「資料準備中」，真正的影像列要等確認完成才出現 — 舊版只認得另一種「-」狀態，遇到這種情況會誤判成「沒有可下載的圖片」。改成只要整張清單沒有任何一列可抓或可觸發，就自動重抓並輪詢直到清單就緒。代價：真的零影像的個案每次同步多等約 12-15 秒。

---

## 0.17.6 重點 — 2026-06-08

**✍️ 影像下載開關文案 + 無影像時的明確說明**

- 開關文案改為一般民眾看得懂的「一併下載影像圖片（X 光 / 電腦斷層）」，附說明與成本提示
- 同步結果明細補上無影像時的明確通知：未勾選時提示「此次只取得文字報告」；勾了但 NHI 端真的沒有影像檔時，明說只有文字報告，避免使用者誤以為漏抓

---

## 0.17.5 重點 — 2026-06-08

**🔁 首次同步遇「資料確認中」影像清單自動重抓**

從未抓過影像的個案首次同步時，NHI 清單每列狀態都是「-」（伺服器端確認還沒跑完），舊版會整批漏掉、要再跑第二次同步才補得到。現在偵測到未解析列就自動重抓並輪詢至狀態確定；任何失敗都回退原狀，不影響其他資料。

---

## 0.17.4 重點 — 2026-06-08

**🏷️ 沒有 LOINC 的檢驗保留中英雙名**

沒有 LOINC 的檢驗（如 12068C 甲狀腺球蛋白抗體 aTG）原本只輸出中文醫令名，英文檢驗名整個丟失 → app 的英文介面只能顯示中文。現在補一個 his-local-lab coding 承載英文 assaY_ITEM_NAME，中英兩個名字都進 bundle；醫令名照舊逐字保留。

---

## 0.17.3 重點 — 2026-06-06

**🏷️ 檢驗顯示名優先採「檢驗檢查項目名稱」**

三項 CMV 檢驗（14004B / 14048B / 12184C）顯示的是籠統的醫令名（「巨大細胞病毒抗體 酵素免疫法」）而非具體的檢驗項目名（「巨細胞病毒IgG抗體」）。現在 `code.text` 的取名優先序納入 NHI 的檢驗檢查項目名稱（assaY_ITEM_NAME）；A+B 雙通道合併時也會取較完整的中文名。coding 裡的醫令名照舊逐字保留。

---

## 0.17.2 重點 — 2026-06-06

**📄 放射報告多通道重複合併（全半形 + 截斷判定）**

同一份放射報告經 NHI 多個上傳通道送出時，文字常有些微差異（全形／半形、換行、編號空格），甚至其中一個通道的內容被截斷 — 舊版會輸出成兩份報告，app 看起來像「多部位 CT」。現在比對前先做全半形正規化；一份內容是另一份的「嚴格前綴」（截斷版）時，留完整版、丟截斷版 — 臨床內容零損失，真正不同的兩次檢查不會被誤併。輸出的報告內文仍是 NHI 原文，正規化只用於比對。

---

## 0.17.1 重點 — 2026-06-05

**🐞 修同日同碼兩份不同報告被默默丟掉一份**

同一天同一家醫院用同一個 NHI 醫令碼（33070B）申報了頭頸部 CT 和胸部 CT 兩份不同報告，舊版兩者算出相同的資源 ID → bundle 去重時默默丟掉第二份。現在無影像序號可區分時，把報告內文的指紋摺進 ID：內容不同 → 兩份都保留；內容相同 → 照舊合併（跨通道去重行為不變）。

---

## 0.17.0 重點 — 2026-06-05

**📑 新功能 — 出院病摘進 bundle（FHIR DocumentReference）**

住院紀錄現在會一併抓取健康存摺「查看檔案」裡的出院病摘，每次有病摘的住院輸出一份 FHIR R4 DocumentReference：

- 病摘 HTML 原文以 base64 附件完整保留（忠實搬運，不做段落解析）
- 類型標 LOINC 18842-5 "Discharge summary"（loinc.org 查證），並連結到同一次住院的 Encounter
- 同步結果明細在「住院」下新增「N/M 出院病摘」一行
- 本版範圍為 HTML 病摘；PDF 版與段落結構化解析列為後續項目

---

## 0.16.1 重點 — 2026-06-05

**⚡ popup 開啟不再卡頓（大型影像 bundle）+ 步驟畫面不再閃爍重疊**

84 MB 的影像 bundle 會讓 popup 每次打開都先卡 2-3 秒、步驟 2 和 3 短暫疊在一起。修法：

- **儲存拆分**：bundle 中繼資料（檔名／大小／筆數，約 200 B）與資料本體分開存放，開 popup 只讀中繼資料（<10 ms），按「下載」時才讀本體
- **步驟同步還原**：上次停留的步驟先快取，popup 開啟第一幀就還原，不再閃爍
- 順手修掉 backend 模式「後端/本地」對帳面板一直空白的舊 bug

---

## 0.16.0 重點 — 2026-06-05

**🔔 新功能 — 「健保署影像準備中」即時進度 banner**

同步結束時若還有影像在健保署端準備中，popup 會顯示藍色 banner「🖼️ 健保署準備中 · 剩 N 張」，每分鐘自動更新；全部備齊時轉綠色「✅ 影像已備齊」並附一鍵重抓按鈕。可隨時關閉；30 分鐘（登入效期上限）或登入過期自動停止。只計數、不在背景偷抓資料 — 重抓由使用者自己按。

---

## 0.15.7 重點 — 2026-06-05

**👀 「健保署準備中」的影像列不再隱形**

實測發現 11 列影像處於「NHI 已受理、檔案準備中」狀態（status=0），但同步明細所有欄位都顯示 0 — 這些列完全隱形。現在它們會列入「等候健保備齊」，使用者知道過幾分鐘再同步一次就能補到；若狀態剛好已就緒，順手直接抓回。

---

## 0.15.6 重點 — 2026-06-05

**🧹 同一張影像掛兩個醫令碼時合併為一份報告**

醫院有時把同一次檢查拆成兩個申報碼（如 32022C + 32023C 同為「骨盆及髖關節檢查」），影像 bytes 完全相同卻輸出成兩份報告。現在僅當影像內容 byte 完全一致時跨碼合併；不同角度的片子、同日重照的追蹤片內容不同，照舊分開保留。

---

## 0.15.5 重點 — 2026-06-05

**🔧 影像觸發判定簡化 — 只看狀態、不看上傳通道**

延續 v0.15.4：影像「需要觸發準備」的判定也拿掉上傳通道過濾，任何通道只要狀態是「待觸發」就送出準備請求。先前以為 A 通道的待觸發列是「幽靈列」，查明其實是觸發參數 bug（見 v0.15.0 rownum 教訓）。真正備不出來的列由觸發後驗證攔下，不會污染等候清單。

---

## 0.15.4 重點 — 2026-06-05

**🐞 修 A/B 通道的影像被默默丟掉**

實測個案有 3 張已就緒的影像因為掛在 A 通道（非 E 通道）被整批略過。根因：v0.15.0 開發時的測試病人剛好所有影像都在 E 通道，誤把通道當成了影像有無的判定。修正：已就緒（status=1 且有序號）的列不分通道一律抓取 — 上傳通道描述的是「怎麼上傳」，不是「有沒有影像」。

---

## 0.15.3 重點 — 2026-06-05

**🧹 開始新同步時先清掉上次的下載面板**

新同步進行中，下方還掛著上一次的「下載健康紀錄檔」面板，檔名長相一樣、很容易誤按下載到舊資料。現在同步一開始就先清掉，新 bundle 完成時才重新出現。

---

## 0.15.2 重點 — 2026-06-05

**👤 病人預覽加年齡／性別 + CI 型別修正**

- 步驟 2 的病人預覽列補上年齡與性別：「✓ 郭O成 · 66歲 · 男 · P10109****」，欄位沒填就自動略過該段
- 修 v0.15.0 起 backend CI 的 TypeScript 型別檢查失敗（純型別註記，行為不變）

---

## 0.15.1 重點 — 2026-06-05

**✍️ 抓取失敗提醒重新同步 + 影像檔名加 -img 後綴**

- 有影像抓取失敗時，同步結果尾巴現在會明確提醒「再同步一次即可補抓」，不用自己從明細數字裡發現
- 勾選影像下載的 bundle 檔名加 `-img` 後綴（如 `nhi-…-v0.15.1-img.json`），與純文字版（<5 MB vs 30-70 MB）在磁碟上一眼可分

---

## 0.15.0 重點 — 2026-06-05

**🖼️ 大功能 — 影像檢查圖片進 bundle（X 光／超音波／心電圖／CT）**

popup 新增 opt-in「抓影像檢查報告」開關：勾選後，健康存摺裡的影像圖片會以 base64 JPG 附件放進 FHIR 報告（DiagnosticReport.presentedForm），與文字報告合體。

- **直接 API 觸發**：對健保署「尚未備妥」的影像直接呼叫準備 API（每列約 2 秒，取代內部測試版每列 10-15 秒的隱藏分頁點擊法），觸發後輪詢抓回
- **本次沒等到的影像自動接力**：記到暫存清單，下次同步自動補抓（8 天期限）
- **弱網韌性**：分批抓取＋自動重試，實測弱訊號下 20/20 張全數成功
- **影像去重**：同一檢查多通道上傳的相同圖片自動合併，文字報告與影像合體成一份
- **明細透明**：「查看明細」顯示已快取／本次新抓／等候健保備齊／抓取失敗等各狀態張數
- 影像 bundle 可達 30-40 MB，暫存改用 `unlimitedStorage`（1 小時自動清除＋下載即清）
- 同版一併納入 **照護計畫（CarePlan）**：健康存摺「我參與的照護計畫」對應為 FHIR R4 CarePlan，收案／結案日期、醫院、計畫名稱忠實搬運

> 版號說明：v0.14 是影像功能的內部開發迭代（隱藏分頁觸發法），從未正式發布，故本檔由 0.13.5 直接接 0.15.0。
...
- 用 Manifest V3 worker 內建的 `OffscreenCanvas` + `createImageBitmap` 在執行期把紅點合成到既有圖示上，再以 `chrome.action.setIcon` 換上 — **不需額外 PNG 資產**、不需新權限（`action` manifest key 已足夠）
...
**Why**: User-verified anomaly 2026-05-30。長庚嘉義 09006C HbA1c 在健康存摺 raw 上：
...
App dev v0.12.1 報 113 對 "duplicate" 跟 user "verified 健康存摺 只有 1 row" 並列 — 我直接連到 NHI `/api/ihke3000/ihke3409s01/page_load` 看 raw JSON：
...
- 健康存摺真的看到 N row → bundle 必須 emit N obs
...
| 4 multi-reading | 「應該 panel 只 emit 1 筆 obs」 | ❌ **錯** | User 之前 raw audit 已確認健康存摺**真的**每張 panel 2 筆 reading — App dev 沒看 raw。**保留 v0.11.9 G**，不動 |
...
Raw NHI display 仍然完整保留在 `code.coding[nhi].display`（faithful transport — 沒亂改健康存摺給的資料，只是另外提供 SMART app 一個乾淨 label）。
...
「凡是健康存摺上沒有的不能亂加，沒給的不能造，loinc 對應可以」— 這次修法符合：
- LOINC 對應改正（從錯標 14979-9 變正確 63561-5）— ok
- QC filter 廣播（攔下健康存摺裡夾帶的 lab QC 資料，不是 patient data）— ok
...
健康存摺 5/18 raw 顯示 11001C ABO 有兩筆（B 跟 +）、11003C RH 也有兩筆（+ 跟 B）— forward/reverse typing arms 或 dual-antisera 反應，**legit 兩個獨立 readings**。v0.11.8 的 test seed 我誤判成「LIS 重複上傳」，bundle 確實只給每個 panel 1 筆 obs。
...
- G：「擋住健康存摺有但 bundle 沒輸出的 row」這正是 faithful transport 的核心 — 不能 silently 漏 reading
...
User 從 v0.11.6 bundle 看 06013C 尿生化檢查 2026-01-14 → 健康存摺 raw 有 20 row，bundle 只有 11。9 個尿液檢查項目（亞硝酸鹽 / 尿潛血 / 酮體 / 白血球酯脢 / Blood / 等）silent drop。
...
User bug report 2026-05-28：MediPrisma 顯示「Glucose 2000 mg/dL [Negative]」（看起來 critical 高血糖數值），但健康存摺實際是「4+ (2000)」— **`4+` 才是真實的 dipstick 半定量分級，`(2000)` 只是 lab 附的約等於估算值**。Bridge 丟掉了臨床更重要的 grade。
...
User 從 2026-05-25 嘉義長庚 CBC 報告發現「健康存摺顯示 8 個 item，bundle 只有 7 個」。Trace 出 root cause：
...
v0.11.2 的「→ 至 ④ 查看 開啟「醫析 MediPrisma」」CTA 用了藍底 banner 跟其他 panel 的「→ 請切到健康存摺分頁　回 ① 登入 →」hint 同型別卡片，視覺重量太重、跟下面兩張 meta card 競爭注意力。整 panel 三個藍塊堆疊不協調。
...
「→ 請切到健康存摺分頁　回 ① 登入 →」原本在 `取得健康存摺資料` button **下方**，現在改到**上方**：
...
之前 758/758 都沒 dosageInstruction。NHI 健康存摺 IHKE3306S02 **真的沒有獨立的「用法」欄位**，但有「給藥日數」+「給藥總量」。把比值推成文字：
...
FHIR-idiomatic：不依賴 array 順序、generic SMART app 也能 parse。
...
**Critical**
- 「🔒 NHI session 已登出 — 請在 NHI tab 重新登入後再點 Sync」→「🔒 健康存摺登入逾時 — 請回到健康存摺分頁重新登入，然後再按『取得健康存摺資料』」
...
- Reload extension 即可。
- **SMART app 整合 contract 變更**：自己寫過 SMART app 對接的 dev，請更新 Encounter.type 讀法（雙維度 kind + channel，見上方說明）。
...
- 🎨 **CTA 阻擋提示改成藍色 info**：「請切到健康存摺分頁 · 回 ① 登入 →」之前是黃色 alarm 卡片，跟下方黃色免責聲明卡兩塊同色系疊在一起像一塊大警報。改成藍色 info palette，用 `→` 取代 `⚠️` 強化「告訴你下一步」的語意。
...
**UI 重設計**
- 🎨 **Step 4 民眾友善文案**：5 個 bullet（檢驗趨勢 / 用藥時間軸 / 就醫紀錄 / 中英對照 / AI 問答）用具體例子（膽固醇、血糖、肝指數）取代 jargon，痛點 hook 直接戳「健康存摺資料散在各頁面難瀏覽」。
...
**新功能**
- 🩺 **次診斷納入 `Encounter.reasonCode[]`**：之前每筆 Encounter 只有 1 個主診斷；現在從 IHKE3303S02 detail 抓出 `icdcode_data[]` (健康存摺上的「次診斷1、2、3、4」)，每筆 append 為一個 `reasonCode[]` entry，跟主診斷同形狀（ICD-10-CM coding + 雙語 text）。觀察某眼科病人案例最多到 4 個次診斷（青光眼 + 黃斑部皺褶 + 視網膜水腫 + 第三對腦神經麻痺）。**順序契約**：`reasonCode[0]` 永遠是主診斷，後面照 NHI 回傳順序是次診斷 1、2、3...。
...
**新功能**
- 🏥 **藥局 Encounter 改標 `type="藥局"`**：之前 IC 卡的藥局領藥事件跟診所看病一樣標 `"IC卡資料"`，下游 SMART app 沒辦法用結構化欄位區分藥局 vs 診所。本版對藥局事件直接寫 `text="藥局"`、其他事件不變，detection 一行命中。健康存摺申報資料本來就不放藥局，bridge 跟隨這個設計、不合成多餘 Encounter。
...
每個對應均經 loinc.org 逐碼複驗（保留完整 audit 證據）。
...
建議 reload extension 後重新「取得健康存摺資料」，新檔的 lab / imaging 日期就會對。
...
- 🎨 「開啟健康存摺登入」按鈕回歸全站統一藍（先前是 out-of-palette 的 sky）
...
- ✨ **登入預檢**：popup 一開就確認健康存摺分頁有效 session；沒登入會直接跳紅色 banner，不會讓你按完 CTA 才被「🔒 尚未登入」打回票
...
一個 Chrome 擴充功能，可以把你在**健康存摺** (myhealthbank.nhi.gov.tw) 看到的就醫紀錄、用藥、檢驗報告、影像報告，**整理成 FHIR R4 標準格式的健康紀錄檔**。檔案可以：
...
3. 病人資料卡填：
   ...
   - 姓名、完整生日可以隨便填假的
   - 身分證字號**不用填**，按下取得時 extension 會自動從健康存摺帶入
4. 按「**🔄 取得健康存摺資料**」— 通常 15–30 秒（看 NHI 速度）
...
3. 病人資料卡填**姓名 + 出生年份**（性別、完整生日可選；身分證字號會自動從健康存摺帶入）
4. 按「**🔄 取得健康存摺資料**」— 通常 15–30 秒，看 NHI 那邊速度
...
4. 開健康存摺登入 → 點工具列 🏥 圖示 → 填身分證字號 → 按「**📥 同步健康存摺資料**」→ 跑完後按「**📥 下載 FHIR Bundle**」把檔案存到電腦。
...
4. Open NHI 健康存摺, fill in patient ID, click 「📥 同步健康存摺資料」. A FHIR JSON file downloads to your machine.
