...
- **不再自算高低/正常**:先前在健保署沒給異常旗標、但範圍乾淨時,bridge 會自己拿「值 vs 範圍」算出偏高/偏低/正常。現在**完全不做這個計算**(怕範圍解析錯就判錯)。
- **異常判讀只來自健保署自己的訊號**:① 異常旗標(異常/正常)② 健保署明寫的「正常/異常」文字。兩者都沒有 → **留白不猜**。
- 影響:健保署沒標記的檢驗不再出現自算紅字(v1.0.17 殘留的少數偏高/偏低 → 留白)。數值與範圍原文照常保留,app/醫師可自行判讀。

## 1.0.17 — 2026-06-30（檢驗參考範圍:年齡分層不再誤判,異常以健保署標記為準）

純解析修正,不改任何病人數值。

- **修正年齡分層參考範圍被誤解析**:部分醫院的 CBC 參考範圍是按年齡分組的字串(新生兒/嬰兒/兒童/成人各一段)。先前會把第一段「0–14 天」的年齡誤抓成範圍 `0–14`,導致每筆正常 CBC(如血小板 208)都被標成「超出範圍」。現在這種範圍**完整保留原文、不再硬擠出數字上下限**,下游 app 可自行依年齡判讀。
- **異常/正常改以健保署資料本身的標記為準**:只要健保署有標記異常/正常,一律採用它的判定(異常 A / 正常 N),不再用「數值 vs 範圍」自行推算(那在年齡分層範圍會出錯)。健保署沒標記、又是無法化約的年齡分層範圍 → **留白不猜**,而不是給一個假的「偏高」。
- 實測:一份 bundle 中 504 筆 CBC 的假紅字消失;有健保署異常標記的檢驗則精準顯示。

## 1.0.16 — 2026-06-30（檢驗關聯改用「就醫實際申報的檢驗清單」,不再用日期硬猜）

純關聯修正,不改任何病人數值。

- **修正住院檢驗醫令碼比對**:住院明細的檢驗醫令碼是「08011C||08011C」雙寫格式,先前對不上 → 住院做的檢驗連不到住院、被堆到同日門診。現在正確解析。
- **改用「就醫實際申報的檢驗清單」連結,不再用日期猜**:一筆檢驗只連到「醫令清單裡列了它」的那次就醫;對不到任何清單的就**留白(獨立)**,不再硬塞到同日門診。(健保申報只列實際做的檢驗,所以清單可靠;對不到寧缺勿錯。)
- **同一筆檢驗被門診+住院兩邊申報時(如化療入院當天的 CBC)→ 連到涵蓋採檢日的那次住院**(FHIR 上,檢驗屬「採檢當下那次就醫」;住院期間採的血即屬該次住院)。
- 實測:化療入院當天 38 筆檢驗,從「全堆門診」修正為「全連住院」。
...
▍檢驗報告連到「開單的那一診」
- 先前同一天、同一家醫院有多個門診時,檢驗報告無法判斷是哪一診開的,常被堆到不相關的那一診(實例:同日有「攝護腺門診」與「慢性腎臟病門診」,抽血其實是腎臟科開的,卻顯示在攝護腺那診)。現在依該次就診**實際開立的檢驗醫令**,把檢驗連到正確的就診。
- 對不上任何就診醫令清單的少數獨立檢驗,維持不歸屬、不亂塞(忠實搬運)。

▍一次就醫被拆成多筆申報 → 正確合併成「一次就診」
- 健保有時把一次就醫的費用拆成多筆申報(例如檢驗一筆、診察+藥一筆),各自的部分負擔與點數不同 —— 先前會被當成多次就診。現在用健保的**就醫序號**判定為同一次,合併成一次就診(檢驗與用藥都歸到同一診)。
- 只在能**確定是同一次就醫**時才合併;序號格式特殊(IC卡序號)或缺失時一律維持原本行為,**不會誤併**已正確分開的就診。
